...
if(window.innerWidth > 992){
    ...
    const img = Array.from(document.querySelectorAll('.product .slider .slide img'));
    img.forEach(img => {
...
addBtn.addEventListener('click', () => {
    ...
    descountPercentage = parseFloat(product.querySelector('.price .percentage').textContent.split('%')[0]),
    originalPrice = parseFloat(product.querySelector('.price .original').textContent.split('$')[1]),
    amount = parseFloat(product.querySelector('.amount .num').textContent);

    productData = {
        name,imgSrc,descountPercentage,originalPrice,amount
    }
...
const addToCart = (data) => {
    const descountedPrice = (data.originalPrice * (data.descountPercentage/100)).toFixed(2);
...
const amountBtns = Array.from(document.querySelectorAll('.product .cart-btns .amount button'));
const amountNumWrapper = document.querySelector('.product .cart-btns .amount .num');
let amountNum = 0;
